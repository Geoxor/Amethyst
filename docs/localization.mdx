...
<Steps>
  <Step title="First Step" icon='github'>
    Follow the cloning procedure stated in [Development](/development)
  </Step>
  <Step title="Second Step" icon='magnifying-glass'>
    Navigate to `src/renderer/locales`
  </Step>
  <Step title="Third Step" icon='file-plus'>
    Find or create a [JSON](https://en.wikipedia.org/wiki/JSON#Syntax) file for the locale you want to localize for are instructions or content that only pertain to the third step.
  </Step>
</Steps>
...
It is expected that certain terminology only makes sense in English, for that reason when localizing try to maintain coherence of meaning in your language even if requires indrect translations or rephrasing

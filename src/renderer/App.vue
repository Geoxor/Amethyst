...
<template>
  ...
  >
    ...
    <div
      v-if="amethyst.getCurrentPlatform() == 'mobile' && !amethyst.state.settings.appearance.desktopMode"
      ...
      >
        <div class="p-2 rounded-32px flex bg-surface-700 justify-between items-center h-50px">
          ...
          <navigation-button
            ...
            class="transform-gpu box-content border-solid border-4px border-surface-900 bg-surface-700 max-w-9 mx-3 rounded-full z-1 scale-175"
            mobile
          />
    ...
    <div class="h-full whitespace-nowrap flex flex-col justify-between">
      ...
      <playback-controls
        v-if="amethyst.state.settings.appearance.showPlaybackControls && amethyst.getCurrentPlatform() !== 'mobile' || amethyst.state.settings.appearance.desktopMode"

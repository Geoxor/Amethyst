...
<template>
  <div class="min-w-64 max-w-64 p-2 pb-4 flex h-full font-cozette overflow-hidden overflow-y-auto " @keypress.prevent>
    <ul class="w-full">
      ...
      <li
        v-for="(song, i) of state.queue" :key="song" :class="[isHoldingControl ? 'cursor-external-pointer' : 'cursor-default', i === state.currentlyPlaying && 'text-blue-500']" class=" h-3 mb-0.5 hover:text-blue-300"

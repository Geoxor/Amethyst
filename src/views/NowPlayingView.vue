...
<template>
  <div class="flex flex-col justify-end h-full items-center w-full text-text-title">
    <template v-if="currentTrack?.getCover() && !showVectorscope">
      <cover-art
        :url="currentTrack?.getCover()"
        class="w-full h-auto top-1/6 opacity-50 filter blur-48 absolute-xy rounded-8px transform-gpu scale-125 "
      />
      <div
        :url="currentTrack?.getCover()"
        class="w-full h-64 bg-gradient-to-t absolute-xy top-32 from-transparent  to-surface-900"
      />
    </template>

    <div class="absolute-xy top-5/16 flex z-1 flex-col items-center gap-4">
      <cover-art
        v-if="!showVectorscope"
        :url="currentTrack?.getCover()"
    ...
    <div class="w-7/8 flex gap-2 flex-col">
      ...
      <div class="flex justify-between">
        <title-text :text="amethyst.player.currentTimeFormatted(true)" />
        <title-text :text="currentTrack?.getDurationFormatted(true) || ''" />
      </div>
    </div>
